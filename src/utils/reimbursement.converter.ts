...
export function convertReimbursement(row: any) {
    return new Reimbursement(row.reimbursementid,
            new User(row.author, row.author_username, row.author_password && '', row.author_firstname, row.author_lastname, row.author_email,
                row.author_roleid && new Role(row.author_roleid, row.author_role)), row.amount, row.datesubmitted, row.dateresolved, row. description,
            ...
            new ReimbursementStatus(row.statusid, row.status), row.typeid && new ReimbursementType(row.typeid, row.type_name));
}